...
int do_movi(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	start_blk = image[i].start_blk;
	blkcnt = image[i].used_blk;
	printf("%s %s device %d Start %ld, Count %ld ", rw ? "writing" : "reading", image[i].description, dev_num, start_blk, blkcnt);
	sprintf(run_cmd, "mmc %s %d 0x%lx %d %d",	rw ? "write" : "read", dev_num,	addr, start_blk, blkcnt);
	printf("\n%s\n", run_cmd);
	run_command(run_cmd, dev_num);
	printf("completed\n");
	return 1;
